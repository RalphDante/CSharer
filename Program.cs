    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var aiService = new AIService(
                settings.APIs.Groq.ApiKey, 
                settings.APIs.Groq.Model
            );
            
            var bufferService = new BufferService(
                settings.APIs.Buffer.AccessToken, 
                settings.APIs.Buffer.ProfileIds
            );
            
            var pinterestService = new PinterestService(
                settings.APIs.Pinterest.AccessToken, 
                settings.APIs.Pinterest.BoardId
            );
